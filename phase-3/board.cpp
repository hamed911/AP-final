...
void board::show_list(string command,ostringstream& out){
	check_existence_security();
	check_no_more_input( command);
	if(_list.size()<1)
		throw "This board has no list!";
	for(int i =0; i< _list.size() ; i++ )
		_list[i].show_list( out);
}

void board::add_list(string command,ostringstream& out){
	check_existence_security();
	string name_new_list = get_one_input(command);
	for(int i=0; i<_list.size(); i++)
	...
	out<<"You added list < " << name_new_list << "> with number " << list_counter << " to board : " << board_name << " !" << endl;
	list_counter++;
}

void board::remove_list(string command,ostringstream& out){
	check_existence_security();
	string list_numb = get_one_input(command);
	...
	out << "You remove list with number " << remove_list << " successfully!" << endl;
}
...
void board::add_new_card(string command,ostringstream& out){
	check_existence_security();
	vector<string>information = get_two_input(command);
	...
	out<<"You added a new card with number : " << card_counter -1 << " , to list " << l_number <<" successfully!" << endl;
}
...
void board::rename_card(string command,ostringstream& out){
	check_existence_security();
	vector<string> information = get_two_input(command);
	...
	out<< "You changed name of card " << c_number << " to " << information[1] << " !" << endl;
}

void board::edit_card_description(string command,ostringstream& out) {
	check_existence_security();
	vector<string> information = get_two_long_input(command);
	int c_number = atoi (information[0].c_str() );
	if( !c_number )
		throw "You have to enter the number of card correctly!";
	card* edited;
	...
	out<< "You changed description of card " << c_number << " to " << information[1] << " !" << endl;
}

void board::comment(string command,ostringstream& out) {
	check_existence_security();
	vector<string> information = get_two_long_input(command);
	int c_number = atoi (information[0].c_str() );
	if( !c_number )
	...
	out<< "Your comment added to card " << c_number << " !" << endl;
}
...
void board::remove_special_card(string command,ostringstream& out){
	check_existence_security();
	string card_num = get_one_input(command);
	int c_number = atoi (card_num.c_str() );
	if (!c_number )
		throw "You have to enter a number of card";
	if( remove_card (c_number) )
		out << "You removed card " << c_number << " successfully!"<< endl;
}
...
void board::move_card(string command,ostringstream& out){
	check_existence_security();
	vector<string> information = get_two_input(command);
	int c_number = atoi (information[0].c_str() );
	if( !c_number )
	...
	if( remove_card(c_number) ){
		...
		out<< "You move card with number: " << c_number << " to list " << l_number << " successfully!" << endl;
	}
}

void board::assign_user(string command,ostringstream& out){
	check_existence_security();
	vector<string> information = get_two_input(command);
	int c_number = atoi (information[0].c_str() );
	if( !c_number )
	...
	for ( int i=0; i< members.people.size() ; i++)
		...
		out<< "You assigned user to card " << c_number << " !" << endl;
		}
		if(!user_existence)
			throw "This user didn't exist in this board!";
}

void board::remove_user(string command,ostringstream& out) {
	check_existence_security();
	vector<string> information = get_two_input(command);
	int c_number = atoi (information[0].c_str() );
	if( !c_number )
	...
	for ( int i=0; i< edited->worker.size() ; i++)
		if ( edited->worker[i].username == information[1] ){
			...
			out << "You removed "<< information[1] << " from card " << c_number << " !" << endl;
		}
	if ( !user_existence )
		throw "This user didn't exist !";
}

void board::edit_card_due_date(string command,ostringstream& out){
	check_existence_security();
	vector<string> information = get_two_input(command);
	int c_number = atoi (information[0].c_str() );
	if( !c_number )
	...
	if ( !edited )
		throw "There is no card with this number";
	edited->date = _date;
	out<< "You changed date of card " << c_number << " to " << _date << " !" << endl;
}

void board::filter(string command,ostringstream& out){
	check_existence_security();
	vector<string> information =get_filter_input( command);
	...
	for(int i = 0; i< finded_cards.size() ; i++ )
		out << finded_cards[i]->name << " : " << finded_cards[i]->card_number << endl;
}

void board::show_cards_of_list(string command,ostringstream& out){
	check_existence_security();
	string list_name = get_one_input(command);
...
bool board:: handle_board_event(string command,ostringstream& out){
string order;
stringstream ss;
ss << command;
ss >> order;
	if( order == "show_lists" ){
		show_list( command,out);
		return true;
	}
	else if( order == "add_list"){
		add_list( command, out);
		return true;
	}
	else if( order == "remove_list"){
		remove_list( command,out);
		return true;
	}
	else if( order == "move_card"){
		move_card(command,out);
		return true;
	}
	else if( order == "add_card"){
		add_new_card(command,out);
		return true;
	}
	else if( order == "remove_card"){
		remove_special_card( command, out);
		return true;
	}
	else if( order == "rename_card"){
		rename_card(command,out);
		return true;
	}
	else if( order == "show_cards"){
		show_cards_of_list( command,out);
		return true;
	}
	else if( order == "edit_card_description"){
		edit_card_description( command, out);
		return true;
	}
	else if( order == "edit_card_due_date"){
		edit_card_due_date( command, out);
		return true;
	}
	else if( order == "assign_user"){
		assign_user( command, out);
		return true;
	}
	else if( order == "remove_user"){
		remove_user( command, out);
		return true;
	}
